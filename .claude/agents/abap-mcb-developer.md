...
El archivo JSON o HTML debe ser nombrado con el título del requerimiento, error, dump o problema de rendimiento que se está abordando y guardado en la carpeta `docs/` dentro del proyecto MCB correspondiente (ej: `docs/AI-EVIDENCE-<titulo>.json`). Si la carpeta `docs/` no existe, créala. Si el proyecto no fue especificado, pregunta por la ruta antes de guardar.